...
function build_image(){
    ...
    docker build --build-arg ${build_args} --no-cache -t ${build_image_name}:${build_image_tag} -f ${build_docker_file} . || exit 1
}
...
function build_image_platform(){
    ...
    local build_docker_file=$4
    local build_args=$5
    local build_platform_arch=$6
    echo "Building image  ${build_image_name}:${build_image_tag} for platform ${build_platform_arch}..."
    docker buildx build --platform linux/${build_platform_arch} --build-arg ${build_args} --no-cache -t ${build_image_name}:${build_image_tag} -f ${build_docker_file}  --load . || exit 1
}
