...
class PipeCouplerServer(pipecoupler_pb2_grpc.PipeCouplerServicer):

    def HandleData(self, request, context):
        print('Received request: request=%s' % (request))
        
        prediction = modelRunner.predict(json.loads(request.content))
        ...
        with grpc.insecure_channel('%s:%s' % (downstreamHosts, grpcport)) as channel:
            ...
            response = stub.HandleData(pipecoupler_pb2.Data(sender='you', ID='', content=json.dumps(prediction)))
...
def serve():
    ...
    pipecoupler_pb2_grpc.add_PipeCouplerServicer_to_server(PipeCouplerServer(), server)
